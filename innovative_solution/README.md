...
| TFT_LCD_源码与结果复现包.zip | 本地源码、结果与作图资料，不含原始数据或模型对象 |

论文由正文模板及结果表生成，以本文的研究叙述展开。修改和舍弃原因单独记录在 [PROJECT_PROCESS.md](PROJECT_PROCESS.md)，针对意见的说明位于review/特征候选与结论定位处理说明.md。项目所有者授权将原始竞赛数据随项目上传至指定私有仓库；源码、模型、数据和有用比较记录一并保留。仓库上传不等于向竞赛平台提交结果，本项目没有官方线上成绩。
